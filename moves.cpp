...
void add_point()  //just adds a point; M 2b recalculated separately
{
        ...
        {
        		M[z][Nc_z[z]-1][i]=0; M[z][i][Nc_z[z]-1]=0;
        ;}
...
n_type modif_add_1(int f2)
{
        int z=(p[Nc-1]).z;
   	  ...
   	  n_type y=G0(p[Nc-1], p_[Nc-1]);
  	     n_type Ml[N_max], Mr[N_max];
        for (int i=0; i<Nc_z[z]; i++)  {Ml[i]=0; Mr[i]=0;}

        n_type r[N_max], l[N_max];
        int k2=-1, K_=-1; do {next(z,K_); k2++; l[k2]=G0(p[K_], p_[Nc-1]); r[k2]=G0(p[Nc-1], p_[K_]);} while (K_<Nc);

        if (f2==1) {r[Nc_z[z]-1]-=a[Nc-1];l[Nc_z[z]-1]-=a[Nc-1];y-=a[Nc-1];}


        {for (int i=0; i<Nc_z[z]-1; i++)
        #pragma vector always
         for (int j=0; j<Nc_z[z]-1; j++)
         {
         	Ml[i]+=M[z][i][j]*l[j];
            Mr[i]+=r[j]*M[z][j][i];
         ;}
         ;}
     		...
     		Ml[Nc_z[z]-1]=-1;Mr[Nc_z[z]-1]=-1;
        {for (int i=0; i<Nc_z[z]-1; i++) y-=l[i]*Mr[i];}         //y is determined
        {for (int i=0; i<Nc_z[z]; i++) Mr[i]/=y;}
        ...
        static int * kk_array= new int [N_max];
		  {int kk=-1; for (int j=0; j<Nc_z[z]; j++) {next(z,kk); kk_array[j]=kk;};}
        for (int q=0; q<n_part; q++) {gm[q]=GM(z,0,Nc_z[z]-1,q, kk_array); mg[q]=MG(z,0,Nc_z[z]-1,q, kk_array);}



        n_type d=beta*M[z][Nc_z[z]-1][Nc_z[z]-1];
		  //fast update of G
        {
        for (int q=0; q<n_part; q++)
        for (int q2=0; q2<n_part; q2++)
...
//t_counter-=clock();
           for (int wn=1; wn<wn_max; wn++)  if(use_wn(wn))
	        for (int q=0; q<n_part; q++) {gm[wn][q]=GM(z,wn,Nc_z[z]-1,q, kk_array); mg[wn][q]=MG(z,wn,Nc_z[z]-1,q, kk_array);}
//t_counter+=clock();
        ;}
