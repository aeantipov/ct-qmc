...
void cluster_define()
{
   ...
   for (int i=0; i<cluster_size; i++)
   {
      ...
      int K=-1;{int kk=-1; for (int j=0; kk!=2*cluster_KN[i] && j<Nc_z[z]; j++) {next(z,kk);K++;}}
      cluster_KM1[i]=K;
      cluster_nz[z]++;
      ...
      K=-1;{int kk=-1; for (int j=0; kk!=2*cluster_KN[i]+1 && j<Nc_z[z]; j++) {next(z,kk);K++;}}
      cluster_KM2[i]=K;   
      cluster_nz[z]++;
   ;}
...
//   {for (int k=0; k<cluster_size; k++)  if (rnd()<0.1) cluster_state[k]=1; else cluster_state[k]=0; return 1;}
   ...
   static int num=int_value("number_of_trials_in_cluster");
	static n_type *  ww=new n_type [num];
   static int ** states;
   {static int f=0; if (f==0)
   {states= new int * [cluster_size]; for (int i=0; i<cluster_size; i++) states[i]=new int [num];f=1;};}
