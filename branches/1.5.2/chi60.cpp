...
complex ********** chi60_array [n_zone][n_zone][n_zone];   //w0, w1, w2, W -> (w0 w0; w1 w1+W; w2+W w2)
void ini_chi2_array();
void ini_chi4_array();
...
complex chi60_irreducible(int z0,int z1,int z2,int w0,int w1,int w2_,int W,int n0,int n0_,int n1,int n1_,int n2,int n2_,double s)
{
...
if (W<0) {static int wm=2*(int_value("number_of_Matsubara_frequencies_for_Gamma4")/2); return conj(chi60_irreducible(z0, z1, z2, wm-1-w0, wm-1-w1, wm-1-w2_, -W, n0, n0_, n1, n1_, n2, n2_, s));}
	...
	complex C=chi60_array[z0][z1][z2][w0][w1][w2_][W][n0][n0_][n1][n1_][n2][n2_]/s;
   // 9 terms 2-4
...
count++; if (count%(n_part*wn_max*(1+31*todo60))!=0) return;   //to mantain N^2 complexity for chi4...
set_GMchi();
...
//   for (int w2_=0; w2_<2*wc_max/2; w2_++)  if (w1-w1_+w2-w2_==0 && w1_-w1>=0 && w1_-w1<2*(wc_max/2))
   {  int w2=w1_+w2_-w1;
     	if (w2>=0 && w2<2*wc_max/2)
      {
   	int W=w1_-w1, w0_=w0; double f=(curr_sgn/prev_weight); complex C=0.;
...
{ double S6=0;
   ...
   {                                            
      	...
      	complex z=chi60_irreducible(z0, z1, z2, w0, w1, w2_, W, n0, n0_, n1, n1_, n2, n2_, s);
...
void ini_chi60_array()
{
   ...
   {
   	chi60_array[z1][z2][z3]=new complex *********[2*wc_max/2];
      for (int w0=0; w0<2*wc_max/2; w0++)
      {
        chi60_array[z1][z2][z3][w0]=new complex ********[2*wc_max/2];
        for (int w1=0; w1<2*wc_max/2; w1++)
        {
      	chi60_array[z1][z2][z3][w0][w1]=new complex *******[2*wc_max/2];
         for (int w2_=0; w2_<2*wc_max/2; w2_++)
         {
	         chi60_array[z1][z2][z3][w0][w1][w2_]=new complex ******[2*wc_max/2];
            for (int W=0; W<2*wc_max/2; W++)
            {
            	chi60_array[z1][z2][z3][w0][w1][w2_][W]=new complex *****[n_part];
               for (int n1=0; n1<n_part; n1++)
               {
                  chi60_array[z1][z2][z3][w0][w1][w2_][W][n1]=new complex ****[n_part];
               	for (int n1_=0; n1_<n_part; n1_++)
                  {
                  	chi60_array[z1][z2][z3][w0][w1][w2_][W][n1][n1_]=new complex *** [n_part];
                     for (int n2=0; n2<n_part; n2++)
                     {
                     	chi60_array[z1][z2][z3][w0][w1][w2_][W][n1][n1_][n2]=new complex **[n_part];
                        for (int n2_=0; n2_<n_part; n2_++)
                        {
                        	chi60_array[z1][z2][z3][w0][w1][w2_][W][n1][n1_][n2][n2_]=new complex *[n_part];
                           for (int n3=0; n3<n_part; n3++)
                           {
                           	chi60_array[z1][z2][z3][w0][w1][w2_][W][n1][n1_][n2][n2_][n3]=new complex [n_part];
