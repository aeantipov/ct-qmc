...
void MC_single_step()
{
   ...
   { //try add
      ...
      keep_state_all(0,0);
		...
		rnd();//for safety
      if (fabs(uc)>rnd()) {add_two(1);ACCEPT_MC_COUNT++;} //accept
		else {add_two(-1);keep_state_all(1);}
   ;}
   else
   { //try remove
   	if (Nc<2) return;
      keep_state_all(0,0); //keep_state(0, p[K].z, p[K+1].z);
		...
		rnd();//for safety
      if (fabs(uc)>rnd()) {remove_two(1,K);ACCEPT_MC_COUNT++;}//accept
		else {remove_two(-1,K);keep_state_all(1);}
   ;}
