...
//	       delete all_weight[i-1];
//	       cout << "Received!" << endl;
//	       delete[] Gtotal_container[i-1];
//	       delete[] Gw0_container[i-1];
	    //   delete weights[i-1];
	...
	if (CTQMC.test(all_finish[i-1])) 
	  {
	    check_finished[i-1] = 1;
            if (finish_recv[i-1]==1) cout << endl << "Walker " << i << " finished. " << endl;
	    finish_recv[i-1]=2;
  	  };     
   ...
   if ( (time_step%output_period==0 && time_step!=0 && check_gw == (CTQMC.getSize()-1)) || (time_step%(100000000000*output_period)==0 && time_step!=0))
   { 
...
//screen output
		 ...
		 CTQMC.getStream()<<"\n\nESTIMATIONS at "<<i<<" steps, after "<<time(NULL)-ti<<" sec.:\nautocorrelation time "<<int(tau)<<"\n";
         	 CTQMC.getStream()<<"variance "<<double((DispG0_sum-DispG0_sum00)*i/(weight_sum*weight_sum))<<"\n";
                 ...
                 CTQMC.getStream()<<"Average pert. order "<<Nc_sum/weight_sum<<"\n"<<flush;
                  jj=1;
            	;}
              ;}
